...
export class RoomController {
	 ...
	 */
	async getRoomsByAccommodationId(req: Request, res: Response, next: NextFunction) {
		try {
			// Tên param (accommodationId) phải khớp với file routes
			const accommodationId = req.params.accommodationId;
			const { startDate, endDate } = req.query;
			const rooms = await roomService.getRoomsByAccommodationId(accommodationId, startDate as string, endDate as string);
	...
	/**
	 * GET /filter-ids
	 * Internal API: Lọc ID theo giá/người
	 */
	...
	async addAmenityToRoom(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const newConfig = await roomService.addAmenityToRoom(roomId, amenityId, {
				note,
			});
	 ...
	 */
	async removeAmenityFromRoom(req: Request, res: Response, next: NextFunction) {
		try {
