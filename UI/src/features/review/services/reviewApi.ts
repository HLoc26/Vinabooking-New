...
import type { AccommodationReviewsResponse, Review, ReviewDto } from "../types/review.types";

const reviewApi = {
	create: (data: ReviewDto) => apiClient.post<ApiResponse<Review>>("/reviews", data).then((r) => r.data.data),

	getByAccommodation: (accommodationId: string) => apiClient.get<ApiResponse<AccommodationReviewsResponse>>(`/reviews/accommodation/${accommodationId}`).then((r) => r.data.data),
